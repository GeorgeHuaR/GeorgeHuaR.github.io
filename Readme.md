...
主题只定义语义 token，不再按具体控件定义变量。组件应优先读取：
...
【V6.3.6】全局 border 语义拆分为 `--border-width` 与 `--border-color-*`，不再定义用 subtle/strong 命名、但实际只承载颜色的复合变量。组件需要完整边框时直接组合宽度、样式和颜色；主题专属风格需要完整 border 时，可在该主题内部定义局部复合变量。
